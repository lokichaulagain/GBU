...
export default function PartyEditDialog({ id, setRefreshNow }: Props) {
  ...
  const [party, setParty] = useState<IPartyOut | undefined>(undefined);
  useEffect(() => {
    const fetch = async () => {
      setIsFetching(true);
      const { data, error, status } = await supabase.from("Party").select().eq("id", id).single();

      if (error) {
        console.error("Failed to fetch party:", error.message);
        setIsFetching(false);
        return;
      }
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setIsUpdating(true);
    const { data, error, status } = await supabase.from("Party").update(values).eq("id", id);
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <p>
          <DialogTriggerAction title="Edit party" />
        </p>
      </DialogTrigger>
