...
export default function PaymentInCreateDialog({ setRefreshNow }: Props) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setIsCreating(true);
    const { data, error, status } = await supabase.from("Payment-in").insert([values]).select().single();

    if (error) {
      toast.error(error.details || "An error occurred during create. Please try again.");
      console.error("Failed to create payment in:", error.message);
      setIsCreating(false);
      return;
    }
        ...
        />
            ...
            <div className=" mt-8 space-x-2">
              <Button
                ...
                {isCreating ? " Please wait" : " Create Payment-in"}
              </Button>
            </div>
          </form>
        </Form>
