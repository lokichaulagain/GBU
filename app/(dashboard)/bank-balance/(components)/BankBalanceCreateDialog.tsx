...
export default function BankBalanceCreateDialog({ setRefreshNow }: Props) {
  // Define your form
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setIsCreating(true);
    const { data, error, status } = await supabase.from("Bank-balance").insert([values]).select().single();

    if (error) {
      toast.error(error.details || "An error occurred during create. Please try again.");
      console.error("Failed to create bank-balance:", error.message);
      setIsCreating(false);
      return;
    }
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button>Create bank-balance</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[725px]">
        <DialogHeader>
          <DialogTitle>Create bank-balance</DialogTitle>
