...
export default function PaymentOutEditDialog({ id, setRefreshNow }: Props) {
  ...
  const [paymentOut, setPaymentOut] = useState<IPaymentoutOut | undefined>(undefined);
  useEffect(() => {
    const fetch = async () => {
      setIsFetching(true);
      const { data, error, status } = await supabase.from("Payment-out").select().eq("id", id).single();
  ...
  useEffect(() => {
    if (paymentOut) {
      form.reset({
        receiptNumber: paymentOut.receiptNumber,
        party: paymentOut.party.toString() || "",
        paymentDate: new Date(paymentOut.paymentDate),
        paidAmount: paymentOut.paidAmount || 0,
        note: paymentOut.note || "",
        paymentMethod: paymentOut.paymentMethod || "",
        image: paymentOut.image || "",
      });
    }
  }, [form, paymentOut]);
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setIsUpdating(true);
    const { data, error, status } = await supabase.from("Payment-out").update(values).eq("id", id);
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <p>
          <DialogTriggerAction title="Edit payment out" />
        </p>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[725px]">
        <DialogHeader>
          <DialogTitle>Update payment-out</DialogTitle>
