...
export default function CashInHandEditDialog({ id, setRefreshNow }: Props) {
  // Define your form
  ...
  useEffect(() => {
    const fetch = async () => {
      setIsFetching(true);
      const { data, error, status } = await supabase.from("Cash-in-hand").select().eq("id", id).single();

      if (error) {
        console.error("Failed to fetch cash-in-hand:", error.message);
        setIsFetching(false);
        return;
      }
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setIsUpdating(true);
    const { data, error, status } = await supabase.from("Cash-in-hand").update(values).eq("id", id);
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <p>
          <DialogTriggerAction title="Edit cash-in-hand" />
        </p>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[725px]">
        <DialogHeader>
          <DialogTitle>Update cash-in-hand</DialogTitle>
          ...
          <form
            ...
            <div className=" mt-8 space-x-2">
              <Button
                ...
                {isUpdating ? " Please wait" : " Create cash-in-hand"}
              </Button>
            </div>
          </form>
        </Form>
