...
export default function IncomeEditDialog({ id, setRefreshNow }: Props) {
  ...
  const [expense, setIncome] = useState<IExpenseOut | undefined>(undefined);
  useEffect(() => {
    const fetch = async () => {
      setIsFetching(true);
      const { data, error, status } = await supabase.from("Expense").select().eq("id", id).single();
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setIsUpdating(true);
    const { data, error, status } = await supabase.from("Expense").update(values).eq("id", id);
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <p>
          <DialogTriggerAction title="Edit expense" />
        </p>
      </DialogTrigger>
          ...
          <form
            ...
            <div className=" mt-8 space-x-2">
              <Button
                ...
                {isUpdating ? " Please wait" : " Update expense"}
              </Button>
            </div>
          </form>
        </Form>
