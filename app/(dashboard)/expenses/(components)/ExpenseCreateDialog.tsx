...
export default function ExpenseCreateDialog({ setRefreshNow }: Props) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setIsCreating(true);
    const { data, error, status } = await supabase.from("Expense").insert([values]).select().single();
          ...
          <form
            ...
            <div className=" mt-8 space-x-2">
              <Button
                ...
                {isCreating ? " Please wait" : " Create expense"}
              </Button>
            </div>
          </form>
        </Form>
