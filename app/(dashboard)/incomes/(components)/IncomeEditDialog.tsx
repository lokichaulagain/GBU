...
export default function IncomeEditDialog({ id, setRefreshNow }: Props) {
  ...
  const [income, setIncome] = useState<IIncomeOut | undefined>(undefined);
  useEffect(() => {
    const fetch = async () => {
      setIsFetching(true);
      const { data, error, status } = await supabase.from("Income").select().eq("id", id).single();
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setIsUpdating(true);
    const { data, error, status } = await supabase.from("Income").update(values).eq("id", id);
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <p>
          <DialogTriggerAction title="Edit income" />
        </p>
      </DialogTrigger>
          ...
          <form
            ...
            <div className=" mt-8 space-x-2">
              <Button
                ...
                {isUpdating ? " Please wait" : " Create income"}
              </Button>
            </div>
          </form>
        </Form>
